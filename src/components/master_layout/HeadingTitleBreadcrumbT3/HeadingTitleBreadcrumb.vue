<template>
  <div id="heading-title-breadcrumb" class="heading-title-breadcrumb">
    <div class="title-section">
      <p class="page-title">{{ routeName_ }}</p>
    </div>
...
export default {
  ...
  created() {
    this.bereadCrumbCreator();
  },
  methods: {
    bereadCrumbCreator() {
      let separator = { sep: " / " };
      for (let i = 0; i < this.pathName.length; i++) {
        this.paths.push(this.pathName[i]);
        if (i + 1 < this.pathName.length) {
          this.paths.push(separator);
        }
      }
    },
    sbuClick(name) {
...
@import url("./HeadingTitleBreadcrumb.less");
/* #sbu-heading {
  padding: 10px 0 0;
}
